...
*/
###############################################################################################################################################################################################################################################################
#### BIG-IP Password Creation #################################################################################################################################################################################################################################
#
# Generates a random password and stores it in an AWS Secret file. Can be used by any deployed BIG-IPs to set the default password.
# 
#
###############################################################################################################################################################################################################################################################
...
resource "aws_secretsmanager_secret" "bigip" {
  name = format("%s-bigip-secret-%s", var.project, random_id.id.hex)
}
...
resource "aws_vpc" "security-vpc" {
  ...
  tags = {
    Name = "${var.project}_Security-VPC"
  }
}
...
resource "aws_subnet" "sec_subnet_internet_region-az-1" {
  cidr_block              = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 0)
  vpc_id                  = aws_vpc.security-vpc.id
  availability_zone       = var.region-az-1
  map_public_ip_on_launch = "true"

  tags = {
    Name = "${var.project}_sec_subnet_internet_region-az-1"
  }
}

resource "aws_subnet" "sec_subnet_egress_to_ch1_region-az-1" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 1)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-1

  tags = {
    Name = "${var.project}_sec_subnet_egress_to_ch1_region-az-1"
  }
}

resource "aws_subnet" "sec_subnet_ingress_frm_ch1_region-az-1" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 2)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-1

  tags = {
    Name = "${var.project}_sec_subnet_ingress_frm_ch1_region-az-1"
  }
}

resource "aws_subnet" "sec_subnet_application_region-az-1" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 3)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-1

  tags = {
    Name = "${var.project}_sec_subnet_application_region-az-1"
  }
}

resource "aws_subnet" "sec_subnet_egress_to_ch2_region-az-1" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 4)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-1

  tags = {
    Name = "${var.project}_sec_subnet_egress_to_ch2_region-az-1"
  }
}

resource "aws_subnet" "sec_subnet_ingress_frm_ch2_region-az-1" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 5)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-1

  tags = {
    Name = "${var.project}_sec_subnet_ingress_frm_ch2_region-az-1"
  }
}
...
resource "aws_subnet" "sec_subnet_mgmt_region-az-1" {
  ...
  tags = {
    Name = "${var.project}_sec_subnet_mgmt_region-az-1"
  }
}

resource "aws_subnet" "sec_subnet_peering_region-az-1" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 101)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-1

  tags = {
    Name = "${var.project}_sec_subnet_peering_region-az-1"
  }
}

#Security VPC Subnets AZ2

resource "aws_subnet" "sec_subnet_internet_region-az-2" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 10)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-2

  tags = {
    Name = "${var.project}_sec_subnet_internet_region-az-2"
  }
}

resource "aws_subnet" "sec_subnet_egress_to_ch1_region-az-2" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 11)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-2

  tags = {
    Name = "${var.project}_sec_subnet_egress_to_ch1_region-az-2"
  }
}

resource "aws_subnet" "sec_subnet_ingress_frm_ch1_region-az-2" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 12)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-2

  tags = {
    Name = "${var.project}_sec_subnet_ingress_frm_ch1_region-az-2"
  }
}

resource "aws_subnet" "sec_subnet_egress_to_ch2_region-az-2" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 14)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-2

  tags = {
    Name = "${var.project}_sec_subnet_egress_to_ch2_region-az-2"
  }
}

resource "aws_subnet" "sec_subnet_ingress_frm_ch2_region-az-2" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 15)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-2

  tags = {
    Name = "${var.project}_sec_subnet_ingress_frm_ch2_region-az-2"
  }
}
...
resource "aws_subnet" "sec_subnet_application_region-az-2" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 13)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-2

  tags = {
    Name = "${var.project}_sec_subnet_application_region-az-2"
  }
}
...
resource "aws_subnet" "sec_subnet_mgmt_region-az-2" {
  ...
  tags = {
    Name = "${var.project}_sec_subnet_mgmt_region-az-2"
  }
}

resource "aws_subnet" "sec_subnet_peering_region-az-2" {
  cidr_block        = cidrsubnet(aws_vpc.security-vpc.cidr_block, 8, 201)
  vpc_id            = aws_vpc.security-vpc.id
  availability_zone = var.region-az-2

  tags = {
    Name = "${var.project}_sec_subnet_peering_region-az-2"
  }
}
...
/*
CREATE INTERNET RT SECURITY-VPC
*/
...
resource "aws_route_table_association" "sec_subnet_internet_region-az-1" {
  route_table_id = aws_route_table.internet_rt.id
  subnet_id      = aws_subnet.sec_subnet_internet_region-az-1.id
}

resource "aws_route_table_association" "sec_subnet_mgmt_region-az-1" {
  route_table_id = aws_route_table.internet_rt.id
  subnet_id      = aws_subnet.sec_subnet_mgmt_region-az-1.id
}

resource "aws_route_table_association" "sec_subnet_internet_region-az-2" {
  route_table_id = aws_route_table.internet_rt.id
  subnet_id      = aws_subnet.sec_subnet_internet_region-az-2.id
}

resource "aws_route_table_association" "sec_subnet_mgmt_region-az-2" {
  route_table_id = aws_route_table.internet_rt.id
  subnet_id      = aws_subnet.sec_subnet_mgmt_region-az-2.id
}
...
resource "aws_route_table_association" "sec_subnet_egress_to_ch1_region-az-1" {
  route_table_id = aws_route_table.to_security_insepction_1_rt.id
  subnet_id      = aws_subnet.sec_subnet_egress_to_ch1_region-az-1.id
}

resource "aws_route_table_association" "sec_subnet_egress_to_ch1_region-az-2" {
  route_table_id = aws_route_table.to_security_insepction_1_rt.id
...
resource "aws_route_table_association" "sec_subnet_ingress_frm_ch1_region-az-1" {
  route_table_id = aws_route_table.frm_security_insepction_1_rt.id
  subnet_id      = aws_subnet.sec_subnet_ingress_frm_ch1_region-az-1.id
}

resource "aws_route_table_association" "sec_subnet_ingress_frm_ch1_region-az-2" {
  route_table_id = aws_route_table.frm_security_insepction_1_rt.id
...
resource "aws_route_table_association" "sec_subnet_egress_to_ch2_region-az-1" {
  route_table_id = aws_route_table.to_security_insepction_2_rt.id
  subnet_id      = aws_subnet.sec_subnet_egress_to_ch2_region-az-1.id
}

resource "aws_route_table_association" "sec_subnet_egress_to_ch2_region-az-2" {
  route_table_id = aws_route_table.to_security_insepction_2_rt.id
...
resource "aws_route_table_association" "sec_subnet_ingress_frm_ch2_region-az-1" {
  route_table_id = aws_route_table.frm_security_insepction_2_rt.id
  subnet_id      = aws_subnet.sec_subnet_ingress_frm_ch2_region-az-1.id
}

resource "aws_route_table_association" "sec_subnet_ingress_frm_ch2_region-az-2" {
  route_table_id = aws_route_table.frm_security_insepction_2_rt.id
...
resource "aws_vpc_endpoint" "s3-security-vpc" {
  vpc_id       = aws_vpc.security-vpc.id
